...
# -3.9852, 41.6669, 828
latitude = 41.66691   # degrees North
longitude = -3.98396  # degrees East (negative for West)
height = 826         # meters above sea level
...
max_h, max_m, max_s = ut_to_hms(result['ut_maximum'])
fc_h, fc_m, fc_s = ut_to_hms(result['ut_first_contact'])
sc_h, sc_m, sc_s = ut_to_hms(result['ut_second_contact'])
tc_h, tc_m, tc_s = ut_to_hms(result['ut_third_contact'])
lc_h, lc_m, lc_s = ut_to_hms(result['ut_last_contact'])
...
duration = (result['ut_third_contact'] - result['ut_second_contact']) * 3600  # seconds
